from app.ai.ai_model import verifica_pergunta, roteador_eitruck, especialista_auto, gemini_resp, juiz_resposta, orquestrador_resp
...
def models_management(user_id,session_id,question):
    ...
    if float(score[0]) <= 0.6:
        ...
        resposta_roteador = roteador_eitruck(user_id,session_id).invoke({"input":question},
        config={"configurable":{"session_id":session}})
        if "ROUTE=" not in resposta_roteador:
            return resposta_roteador
    
        elif "ROUTE=agenda" in resposta_roteador:
            resposta = especialista_auto(user_id,session_id).invoke({"input":resposta_roteador},
                config={"configurable":{"session_id":session}})
        
        elif "ROUTE=financeiro" in resposta_roteador:
            resposta = gemini_resp(user_id,session_id).invoke({"input":resposta_roteador},
                config={"configurable":{"session_id":session}})
    ...
    resposta = juiz_resposta(user_id,session_id).invoke({"input":resposta},
                config={"configurable":{"session_id":session}})
    ...
    resposta = resposta = orquestrador_resp(user_id,session_id).invoke({"input":resposta},
        config={"configurable":{"session_id":session}})
